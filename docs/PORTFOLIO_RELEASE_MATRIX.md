...
Last refreshed: **2026-07-02** (T-41 days to Kickstarter launch Aug 12 2026 / T-35 days to filming window Jul 7 2026).
...
| **ThoxNova** | `thoxnova-12b-agent` Q4_K_M -> `ttracx/thoxnova-agent:core` + `thoxnova-12b-core` Q4_K_M -> `ttracx/thoxnova-core:v1` | LattePanda N100 (Intel x86 CPU / SYCL / Vulkan; locked 2026-06-01, replaces Jetson Orin NX) | ThoxOS Nova firmware (TBD — no release yet, pre-launch) | [`thoxllm-factory` `registry/0.1.6.json`](https://github.com/ttracx/thoxllm-factory/blob/main/registry/0.1.6.json) | [`ttracx/thoxnova-soc`](https://github.com/ttracx/thoxnova-soc) — no release yet, pre-launch (LattePanda N100 provisioner PR #5 in flight; canonical registry entries `thoxnova-12b-agent` + `thoxnova-12b-core` landed in `0.1.6`) | AT RISK — SoC repo has no release, ThoxOS Nova firmware TBD; user action #9 (flash LattePanda + run install.sh) is the launch-critical keyboard step; both `thoxnova-12b-agent` and `thoxnova-12b-core` tags depend on Phase C 12B training fire (see PULL_FORWARD_TRACKER item 14 CLOSED at infra layer 2026-06-29; keyboard step remains) |
...
- **Ship-readiness percent:** **~94%** (aggregate, per `docs/PULL_FORWARD_TRACKER.md` 2026-07-01 EOD third-pass audit; unchanged as of 2026-07-02 morning standup)
...
- **2026-07-02** — Initial cut. Consolidates SKU + model + firmware + release across 15+ repos. Populated from `thoxllm-factory/registry/0.1.6.json` + per-repo tag lists (verified via local `.git`) + `PULL_FORWARD_TRACKER.md` 2026-07-01 EOD headline metrics + 2026-07-02 8:30 AM PT standup. Single most important gap surfaced: **ThoxNova SoC repo (`ttracx/thoxnova-soc`) has no release yet and the ThoxOS Nova firmware is still TBD**. That is the launch-critical hole and it maps directly to user action #9 (flash LattePanda + run install.sh) + Phase C 12B training fire (models `thoxnova-12b-agent` + `thoxnova-12b-core`).
