...
| **G Apps** | thox-terminal v0.3 follow-up: wire the new thox-actions schema into the existing tmux quick-actions + onboarding + THOXY flows so the operator can speak/type natural language and the FunctionGemma model picks the right structured action | dispatched 2026-06-24 (in flight) | tbd |
