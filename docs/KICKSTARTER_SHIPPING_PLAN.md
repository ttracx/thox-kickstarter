...
| **A Marketing** | P0 CRITICAL claim-reconciliation pass on Thox.ai live tree: scrub the residual $89.99 / $629 / $899 Founders Pre-Order / December 2026 / "Founder Reserve" copy from `src/app/*` pages + API routes + Stripe setup scripts + Founders-Campaign/ + kickerstarter/ standalone HTML; archive the obsolete docs under `archive/`; align everything to the Aug 12 2026 4-SKU lineup | dispatched 2026-06-24 (in flight) | tbd |
| **Industrial design** | `ttracx/thox-enclosures` rev-a-v0.2.0: stand up OpenSCAD source-of-truth files for all 3 devices per ADR-003; derive STEP + STL + 3MF from those; run lay-flat + print-audit gate; cut rev-a-v0.2.0 with all 9 Rev A spec features integrated | dispatched 2026-06-24 (in flight) | tbd |
| **Cross-product foundation** | `ttracx/thox-actions` v0.2.0: replace the stub PromptTemplate with the real FunctionGemma JSON tool-call shape + confidence calibration + a smoke-test fixture against the actual model card I/O contract | dispatched 2026-06-24 (in flight) | tbd |
