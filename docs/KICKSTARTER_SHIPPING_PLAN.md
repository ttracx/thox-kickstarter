...
| **Flagship runtime** | NEW `ttracx/thoxcore` repo: user-uploaded TurboLLMmain.zip absorbed as the seed for THOXCore. Locked hierarchy: THOXCore -> {Runtime, Router, Edge, Cloud, Mobile, SDK}; ThoxOS adapter tree -> {Ollama, llama.cpp, vLLM, LiteRT, TensorRT, MLX, OpenAI-Compatible}. Long-term flagship comparable to Ollama / llama.cpp; central runtime layer across ThoxNova + ThoxMini + ThoxClip + ThoxVault + ThoxOS. IP-038. | dispatched 2026-06-24 (in flight) | tbd |
