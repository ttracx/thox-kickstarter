...
| **Flagship runtime reference** | NEW `ttracx/thoxcore-reference` repo: ChatGPT-built production-ready THOXCore 1.4.2-thoxcore.1 (TypeScript/Node, 505/505 tests pass, npm tarball, systemd + Windows scheduled-task ops, web console, HTTP API at /healthz / /api/v1/{status,adapters,stop}). **Code is FSL-1.1-ALv2 inherited; CANNOT redistribute under Apache-2.0**. Repo exists as a versioned reference for porting design patterns into the Rust `ttracx/thoxcore` workspace. License posture locked: design-only, no source copy. | dispatched 2026-06-24 (in flight) | tbd |
| **Adapter wave 1: LiteRT** | `ttracx/thoxcore` v0.2.0 LiteRT adapter wire: replace the stub with real `THOXCoreAdapter` impl backed by `thox-litert-lm` v0.2 Rust port; load FunctionGemma 270M FT mobile actions per the chat_template.jinja pinned in `thox-actions` v0.2.0; first end-to-end utterance -> k:v decode -> ThoxAction smoke test in the THOXCore example pipeline. | dispatched 2026-06-24 (in flight) | tbd |
| **Adapter wave 1: OpenAI-Compatible** | `ttracx/thoxcore` v0.2.0 OpenAI-Compatible adapter wire: real HTTP client against `/v1/chat/completions` (works with Ollama / vLLM / LM Studio / llama-server); reuses the URLProtocol/reqwest patterns from rc7/rc8 in thox-terminal; unlocks every commodity LLM server as a THOXCore backend in one shot. | dispatched 2026-06-24 (in flight) | tbd |
