...
| **Industrial design** | NEW `ttracx/thox-enclosures` repo: user-uploaded accurate-sized prototype STLs (ThoxMini / ThoxMini Air / ThoxClip placeholder shells) + Qidi Q2 print queue preview + Rev A spec doc with the proposed mechanism + battery + Qi2 + MagSafe + carrier-board feature list ready to be implemented in the next CAD pass | dispatched 2026-06-24 (in flight) | tbd |
