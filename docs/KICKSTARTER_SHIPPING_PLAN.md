...
| **Commerce / config UX** | NEW `ttracx/thox-configurator` repo: user-uploaded ThoxMaker.zip absorbed - 6-step wizard (Device, Capacity, Model, Apps, Finish, Review) for ThoxKey + ThoxStick. Server-authoritative pricing (browser never sends price). AI Assist panel re-validated by deterministic engine. Stripe checkout for ThoxKey path; reserve-only for ThoxStick mechanical mockup. 21 TS/TSX files transform clean; Python FastAPI service byte-compiles. 3 Supabase tables (cfg_configurations / cfg_orders / cfg_provisioning_jobs). Optional integration of THOXStick render assets from the second uploaded zip. | dispatched 2026-06-24 (in flight) | tbd |
