...
| **Campaign ops** | NEW `ttracx/thox-ks-monitor` repo: Kickstarter campaign monitoring + post-pledge ops dashboard (Lane A sanctioned CSV import + Lane B opt-in public scrape + Claude AI digest layer + Next.js + Supabase + RLS). Honors KS ToS by NOT storing creator credentials. Built from the prior agent's pre-validated artifact at `Thox_Kickstarter_Integration.zip` | dispatched 2026-06-24 (in flight) | tbd |
| **Edge AI** | NEW `ttracx/thox-edge-skills` repo: Google AI Edge Gallery `SKILL.md` skills targeting Gemma 4 E2B + FunctionGemma 270M FT mobile actions. Mirrors the THOX device fleet (ThoxNova / ThoxMini / ThoxAir / ThoxClip / ThoxStick / ThoxKey / MagStack) as Edge-Gallery-compatible text + JS + native-intent skills | dispatched 2026-06-24 (in flight) | tbd |
